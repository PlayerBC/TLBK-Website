...
| C02 | Drag product photos with a mouse and touch; use arrow keys or Home/End on a focused photo. Save, reopen, remove/upload photos and reorder again. | The Cover badge follows the first photo. Saving persists the array order and updates the customer menu cover/gallery without another deployment. Other unsaved product fields survive reordering. Escape/outside drops cancel a drag, and closing without saving discards the draft order. Staff view cannot reorder. | __________________ |
...
| M17 | Cancel awaiting-payment and under-review orders with a reason; apply/remove a Refund label. Cancel paid orders with and without stock restoration. | Unapproved payment becomes Cancelled and leaves review counts/filters; Refund label changes cannot reopen review. Proof and history remain. Unpaid holds return once. Paid remains Paid; explicit restore returns eligible units, no-restore leaves produced units unavailable. Reasons/staff/time retained. | __________________ |
...
| M20 | Export CSV and print summary; use demo name beginning with `=1+1`. Print a two-line delivery order, a six-line pickup order, and long mixed-box options/instructions. Select multiple orders, select all shown, filter, and clear selection; print five orders on A4 and Letter. | Export includes required order details and quoted values; spreadsheet formula-like input is neutralized. Print preview uses 5-inch-wide × 4-inch-high preparation slips, four per landscape A4/Letter sheet from the top-left, with cutting guides. Slips include photos, saved names/options/quantities/prices, buyer/phone/socials, order instructions and delivery address or pickup details. General customer pickup instructions are omitted. Larger orders use numbered continuation slips; each order's totals appear once on its final slip. Only selected orders print; filtering clears selection, selected details are freshly fetched, and an unavailable order stops the batch. Paper can be chosen before printing. No clipped content, missing text, private notes, proof, history or original-approved-payment figures. Choose the matching paper, landscape, Actual size / 100%, and turn browser headers/footers off. | __________________ |
...
| E13 | Submit valid payment proof with two verified team accounts assigned Owner and Staff. Leave fulfillment reminders off. | Each account receives one individual review email on the next available worker cycle, containing reference, customer name, fulfillment details, saved products/quantities/options/unit and line prices, subtotal/discount/delivery/total, and an authenticated admin-dashboard link. Test both discounted delivery and zero-fee pickup. Customer tokens and private proof URLs are absent. Retrying the request or worker does not duplicate the event. | __________________ |
