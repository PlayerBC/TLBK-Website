...
| M20 | Export CSV and print summary; use demo name beginning with `=1+1`. Print a two-line delivery order, a six-line pickup order, and long mixed-box options/instructions. | Export includes required order details and quoted values; spreadsheet formula-like input is neutralized. Print summary opens A6 landscape (148 × 105 mm) preparation slips with photos, saved names/options/quantities/prices, buyer/phone/socials, instructions and delivery address or pickup details. Larger orders use numbered continuation slips; totals appear once on the final slip. Photos load before printing or show a fallback. No clipped content, missing text, private staff notes, proof, history or original-approved-payment figures. Print at Actual size / 100% with browser headers/footers off. | __________________ |
