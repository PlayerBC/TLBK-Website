...
In **Daily quantities**, select a future fulfillment date in the calendar. Choose a date at least a week away and ensure it is open in settings. Find the product by its photo/name, enter quantity **5**, then click **Save quantities**. Select several dates to apply the same total separately to each date.

Quantity is per product **per fulfillment date**. Blank (no saved limit) means unlimited; entering 0 means no stock. Saved limits appear when reopening a date; clearing a limit makes it unlimited. Different limits across selected dates show Mixed and stay unchanged unless edited. Pickup and delivery share those five boxes. With the cutoff blank, production starts tomorrow: Monday submission with one Tuesday production day means Wednesday at the earliest. With a configured cutoff, the order day counts before the cutoff if it is open for production; at or after the cutoff, counting starts tomorrow. For example, with a 12 PM cutoff and all production days open, a two-day product ordered September 19 is eligible September 21 before noon or September 22 at/after noon. Fulfillment is always after the last counted production day.
