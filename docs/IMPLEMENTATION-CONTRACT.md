...
`cancel_order` payload `{order_id,revision,reason,restore_stock:boolean,idempotency_key}` -> order. Awaiting-payment and under-review payments become `cancelled`, closing review without claiming approval, rejection or a refund. Paid remains Paid; redeemed promo use is not restored; produced quantity returns only on explicit true. Unpaid always releases. Proof, payment reference and audit history remain available. Admin review counts, filters and approval actions require an unrefunded, under-review payment with pending-confirmation fulfillment. The migration repairs existing cancelled/unapproved orders once with a revision increment and audit event, without stock, promo, payment, refund-label or email changes.
