...
export function mountPartyCartPhotos(root, { role, connected, api, upload, page = 'party' }) {
  ...
  function controls() {
    ...
    $('[data-cart-photo-state]').textContent = dirty ? 'Unsaved photo changes' : loaded ? `${items.length} photos Â· ${items.filter(p => p.published).length} visible` : '';
  ...
  root.addEventListener('change', async event => {
    ...
    try {
      for (const [n, file] of files.entries()) {
        message(`Converting and uploading photo ${n + 1} of ${files.length}â€¦`);
        try {
  ...
  root.addEventListener('click', async event => {
    ...
    busy = true; controls(); message('Saving photosâ€¦');
    try {
