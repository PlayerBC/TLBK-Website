import { catalogProductGroups, orderedCatalogProducts } from './catalog-ordering.js?v=drag-order-1';
...
export function mountCatalogOrder(root, { kind, items, categories, editable, api, escapeHtml:esc, safeImage, onSaved }) {
  ...
  const changed=()=>draft.some((item,i)=>item.id!==saved[i]?.id);
  const snapshot=()=>saved.map(item=>({id:item.id,sort_order:item.sort_order||0,...(product?{category_id:item.category_id||null}:{})}));
  ...
  const groupItems=group=>draft.filter(item=>group.items.some(original=>original.id===item.id));
  function controls() {
  ...
  function paint() {
    ...
    const list=container.querySelector(`[data-order-group="${gi}"] .catalog-order-list`), rows=groupItems(group);
  ...
  bindOrderDrag(list,()=>editable && !busy,(from,to)=>{
    const rows=groupItems(group), item=rows[from], source=draft.indexOf(item), target=draft.indexOf(rows[to]);
    draft.splice(source,1); draft.splice(target,0,item); paint();
  ...
  root.addEventListener('click',async event=>{
    ...
    if (button.hasAttribute('data-order-reset')) { draft=structuredClone(saved); paint(); status.className='muted'; status.textContent='Saved order restored.'; return; }
    if (!editable || !changed()) return;
    busy=true; controls(); status.className='muted'; status.textContent='Saving order...';
    try {
      const result=await api('reorder_catalog',{kind,ids:draft.map(item=>item.id),expected:snapshot()});
      saved=structuredClone(result.items); draft=structuredClone(result.items); paint();
