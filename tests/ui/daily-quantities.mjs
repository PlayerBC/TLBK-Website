...
try{
 ...
 await day(next).click();check('Different saved limits show Mixed',await input(0).getAttribute('placeholder')==='Mixed');
 ...
 check('Failed saves retain edits and restore editable controls',await input(3).inputValue()==='3'&&!await input(3).isDisabled());
 ...
 for(const date of [today,next,'2026-10-02'])await page.locator(`[data-calendar-remove="${date}"]`).click();
